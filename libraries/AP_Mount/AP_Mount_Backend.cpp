...
{
    float GPS_vector_x = 1, GPS_vector_y = 1;

    if (target.flags.relative_xy){
        // AP_Mount does not support the relative location of ardupilot.. This only effects pan calculations
    }else{
        GPS_vector_x = (target.lng-_frontend._current_loc.lng)*cosf(ToRad((_frontend._current_loc.lat+target.lat)*0.00000005f))*0.01113195f;
        GPS_vector_y = (target.lat-_frontend._current_loc.lat)*0.01113195f;
    }
